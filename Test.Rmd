...
timeClients = rep(sample(timeClientsSample, length(id), replace = TRUE))
...
wagesPerYearLostSampFirst = c(0:50000)
wagesPerYearLostSampLast = c(0:80000)
lengthFirst = round(length(id),0)
lengthLast = length(id)-lengthFirst

wagesPerYearLostFirst = sample(wagesPerYearLostSampFirst, lengthFirst)
wagesPerYearLostFirst = wagesPerYearLostFirst*reductSucRate

wagesPerYearLostLast = sample(wagesPerYearLostSampLast, lengthLast)
wagesPerYearLostLast = wagesPerYearLostLast*reductSucRate

wagesPerYearLost = c(wagesPerYearLostFirst, wagesPerYearLostLast)
...
# Assume that we are working with adults who range in age from 18 to 64, not 65 because that can produce zeros.  
# Then get the differnece between the age and 65 and that is the number years for the lost wages
timeWagesSamp = c(18:63)
...
# I want to rep the wage sequence for wages lost per year, because that grows over time
benefitsWagesRate = NULL
for(i in 1:length(id)){
  benefitsWagesRate[[i]] = npv(r = .03, rep(wagesPerYearLost, timeWages[[i]]))
}
...
medicalCostsMen = round(rnorm(mean = 42490, sd = 1000, n = length(id)),2)
medicalCostsMen = round(medicalCostsMen*costDat$maleServed,2)
...
BCAdatGen()
```
Now get a confidence interval for BCA ratio
```{r}
n = 100
BCARatioValues = replicate(n, BCAdatGen())
BCARatioValuesMean = mean(BCARatioValues)
BCARatioValuesSE = 2*sd(BCARatioValues)/sqrt(n)
